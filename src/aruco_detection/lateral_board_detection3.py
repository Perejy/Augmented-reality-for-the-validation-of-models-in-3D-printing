...
def detect_lateral_aruco_board3(image, cam_matrix, dist_coeffs, marker_length=0.07, refind_strategy=False):
    """
    ...
    obj_points = np.array([
        ...
        [0.20, 0, 0], [0.27, 0, 0], [0.27, 0.07, 0], [0.20, 0.07, 0],  # Esquina superior derecha
        [0, 0.39, 0], [0.07, 0.39, 0], [0.07, 0.46, 0], [0, 0.46, 0],  # Esquina inferior izquierda
        [0.20, 0.39, 0], [0.27, 0.39, 0], [0.27, 0.46, 0], [0.20, 0.46, 0]  # Esquina inferior derecha
    ], dtype=np.float32)

    # IDs de los marcadores
    ids = np.array([6, 7, 8, 9], dtype=np.int32)
